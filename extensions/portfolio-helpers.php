 ...
 */
function puredemo_template_chooser( $template ) {
...
add_filter( 'template_include', 'puredemo_template_chooser' );
 ...
 */
function puredemo_body_class( $classes ) {
	...
	if (
		...
		if ( puredemo_get_option( 'portfolio_sidebar', false ) ) {
...
add_filter( 'body_class','puredemo_body_class' );

/**
 * Helper function for displaying image
 */
function puredemo_display_image() {

	// Don't display images on single post if the option is turned off
	if ( is_single() && !puredemo_get_option( 'portfolio_images', true ) ) {
		return;
	}
	...
	<div class="portfolio-image">
		...
		<?php } ?>
		<?php if ( puredemo_get_option( 'layout' ) == 'layout-1col' ) {
